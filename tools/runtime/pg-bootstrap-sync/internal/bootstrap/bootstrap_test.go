...
func TestBootstrapper_Bootstrap_DryRun(t *testing.T) {
	// Create a temp dump file with valid pg_dump content
	tmpDir := t.TempDir()
	dumpFile := filepath.Join(tmpDir, "test.sql")

	// Use PostgreSQL pg_dump format
	dumpContent := `--
...
INSERT INTO public.users VALUES (2, 'Jane Smith');
`
...
func TestBootstrapper_Bootstrap_WithCopyFormat(t *testing.T) {
	// Test PostgreSQL COPY format which is common in pg_dump
	tmpDir := t.TempDir()
	dumpFile := filepath.Join(tmpDir, "test.sql")
...
`
	...
	// Should have processed the CREATE TABLE and COPY data
	if stats.StatementsRead == 0 {
		t.Error("expected StatementsRead > 0")
	}
	if stats.ChangesGenerated == 0 {
		t.Error("expected ChangesGenerated > 0 from COPY data")
	}
}
