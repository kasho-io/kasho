...
3. Set up development env vars

```bash
copy ./environment/development/env.sample to ./environment/development/.env
```
...
```bash
docker exec development-postgres-primary-1 psql -U postgres -d primary_db -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO kasho;"
```
