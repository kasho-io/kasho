...
<div className="alert alert-warning mb-4">
  ...
  <div>
    <div className="font-semibold">SUPERUSER Required</div>
    <div>The replica user requires SUPERUSER privileges to set `session_replication_role = 'replica'`, which prevents triggers from firing during replication.</div>
  </div>
</div>
...
docker run --rm kasho:latest /app/scripts/prepare-dbs.sh
```
...
<div className="alert alert-info mb-4">
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="stroke-current shrink-0 w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
  <div>
    <div className="font-semibold">Security Consideration</div>
    <div>The replica user requires SUPERUSER privileges. In production environments, this should be carefully evaluated. Consider using a dedicated database instance for the replica if security policies restrict SUPERUSER access.</div>
  </div>
</div>
