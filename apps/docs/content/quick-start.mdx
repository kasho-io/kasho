...
All Kasho components are packaged in a single Docker image available from GitHub Container Registry:
- Image: `ghcr.io/jeffrey/kasho:latest` (or specific version like `v1.0.0`)
- Commands: `./pg-change-stream` and `./pg-translicator`

<Callout type="info">
  **Authentication Required**
  
  GitHub Container Registry requires authentication. Run:
  ```bash
  docker login ghcr.io -u YOUR_GITHUB_USERNAME
  ```
  Use a personal access token with `read:packages` scope as the password.
</Callout>
...
<Callout type="info">
  **License Required**
  ...
  Kasho requires a valid license file to operate. See the [Licensing](/installation/licensing) guide for details. If you're interested in becoming a design partner, contact [hello@kasho.io](mailto:hello@kasho.io).
</Callout>
...
- GitHub account with access to pull from GitHub Container Registry
...
services:
  ...
  pg-change-stream:
    image: ghcr.io/jeffrey/kasho:latest
